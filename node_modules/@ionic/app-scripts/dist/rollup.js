...
function rollupWorker(context, configFile) {
    return new Promise(function (resolve, reject) {
        ...
        rollupBundler.rollup(rollupConfig)
            ...
            var bundleOutput = bundle.generate({
                format: rollupConfig.format
            });
            // write the bundle
            var jsFileToWrite = path_1.join(context.buildDir, rollupConfig.dest);
            var promises = [];
            promises.push(helpers_1.writeFileAsync(jsFileToWrite, bundleOutput.code));
            if (bundleOutput.map) {
                promises.push(helpers_1.writeFileAsync(jsFileToWrite + '.map', bundleOutput.map));
            }
